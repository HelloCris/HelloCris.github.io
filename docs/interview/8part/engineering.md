...
| ------------ | ------------------------------------ | ----------------------------------------------------------------- |
| 角色         | 依赖声明文件                         | 依赖锁死文件                                                      |
| 谁来写       | 开发者手写 / `npm install xx` 自动加 | npm 自动生成，**不要手改**                                        |
| 版本写法     | 允许范围，如 `"^18.2.0"`、`"~4.1.0"` | 精确到补丁，如 `"18.2.0"`                                         |
| 内容         | 仅直接依赖 + 脚本 + 元信息           | 完整依赖树（含所有**传递依赖**及精确版本、下载地址、完整性 hash） |
| 可读性       | 高，给人看                           | 低，给机器看                                                      |
| 是否提交 git | 建议提交                             | **必须提交**（团队/CI 一致性的关键）                              |
| 删除后果     | 项目信息丢失                         | 下次 `npm install` 可能装到不同子版本                             |

::: warning ⚠️ 注意

- **`npm install` vs `npm ci`**：`install` 会按 package.json 更新 lock（可能升级小版本）；`ci` 严格按 lock 安装、不许不一致，快且可复现，适合流水线。
- **只改 package.json 不提交 lock 会怎样**：别人装到的版本可能和你不同，依赖漂移。
- **yarn / pnpm 对应文件**：`yarn.lock`、`pnpm-lock.yaml`，作用一样。
- **`^` 和 `~` 区别**：`^1.2.3` 允许 `1.x.x`（不改首个非零位）；`~1.2.3` 允许 `1.2.x`（只允许补丁位变动）。
