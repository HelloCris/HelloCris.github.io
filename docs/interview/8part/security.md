...
**一把钥匙开一把锁**——加密和解密用的是同一把密钥。

- **特点**：速度快、计算开销小，适合加密大量数据
- **常见算法**：AES、DES、3DES
- **前端场景**：
  - 加密本地存储的敏感数据（如 token、用户信息）
  - 大文件上传前的客户端加密
  - WebSocket 通信中的消息体加密

```js
// 伪代码示意
const encrypted = AES.encrypt("敏感数据", "同一把密钥");
const decrypted = AES.decrypt(encrypted, "同一把密钥");
```

**痛点**：密钥怎么安全地传给对方？如果密钥在传输中被截获，一切白搭。

:::

::: info 非对称加密

**一对钥匙**——公钥加密，私钥解密（或者反过来，私钥签名，公钥验签）。

- **特点**：安全性高，但计算慢，不适合加密大数据
- **常见算法**：RSA、ECC（椭圆曲线）
- **前端场景**：
  - 登录时加密密码传给后端（用后端公钥加密）
  - 数字签名验证请求来源
  - HTTPS/TLS 握手阶段协商对称密钥

```js
// 伪代码示意
const encrypted = RSA.encrypt("密码", 服务端公钥);
// 只有服务端持有私钥，才能解密
```
...
> 非对称加密负责安全地交换密钥 → 对称加密负责后续大量数据的加解密

这就是 HTTPS 的工作原理：

1. 浏览器和服务器通过非对称加密（RSA/ECDH）协商出一个**对称密钥**
2. 之后的数据传输全部用这个对称密钥加密（AES-GCM 等）
