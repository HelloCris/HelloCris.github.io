...
- 引用计数算法（老IE）：每个对象都有一个引用计数器，当有变量引用该对象时，计数器就会增加，当没有变量引用该对象时，计数器就会减少。当计数器为 0 时，该对象就会被销毁。
- 标记清除算法：首先从根对象（GC Roots）出发，标记出所有正在使用（可达）的对象，然后遍历内存，清除掉所有未被标记的垃圾对象。

:::

### 附6: for/in 和 Object.keys()

`for...in` 和 `Object.keys()` 都常用来“获取对象的属性”，但一个是**遍历语句**、一个是**返回数组的静态方法**，行为差异主要体现在「是否包含原型链」「是否包含 Symbol / 不可枚举属性」上。

| 对比维度         | `for...in`                                                    | `Object.keys(obj)`                                       |
| ---------------- | ------------------------------------------------------------- | -------------------------------------------------------- |
| 返回/产出        | 逐个拿到 key，在循环体内使用                                  | 返回**自身可枚举属性名**组成的 `string[]`                |
| 包含原型链属性   | ✅ 会遍历原型链上的可枚举属性                                 | ❌ 只取对象**自身**属性                                  |
| 包含 Symbol 键   | ❌ 不包含                                                     | ❌ 不包含                                                |
| 包含不可枚举属性 | ❌ 不包含（除非通过 `defineProperty` 设为 `enumerable:true`） | ❌ 不包含                                                |
| 遍历顺序         | 不保证顺序（整数-like key 会优先）                            | 与 `Object.keys` 规范一致：整数 key 升序，其余按插入顺序 |
| 能否拿到 value   | 需通过 `obj[key]` 访问                                        | 数组里只有 key，value 仍需 `obj[key]`                    |
| 常见用途         | 遍历普通对象的属性                                            | 拿到 key 数组后做 `map` / `filter` 等                    |
